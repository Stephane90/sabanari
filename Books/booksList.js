function searchBooks(event, pageCount, currentPage, paginationLimit){
    ...
    console.log("test")
    let books = []
    fetch(`https://www.googleapis.com/books/v1/volumes?q=${input}&max-results=${paginationLimit}&start-index=${currentPage}`)
        ...
        .then(data => { data.items.map((item) => {
            ...
            document.getElementById("display-books").innerHTML += "<div> title : "+book.title+
                ...
                "</div><img src=\""+book.images.smallThumbnail+"\"/><br/><br/>"
            return book
        })
